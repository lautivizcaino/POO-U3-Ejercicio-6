...
class ListaVehiculos:
    ...
    def insertarElemento(self):
        posicion=int(input('Ingrese la posicion de la coleccion: '))
    ...
    def mostrarElemento(self):
        ...
        if posicion>=0 and posicion<=self.__cantidad:
            contador=0
            if posicion==0:
                print(self.__comienzo.getVehiculo())
            else:
        ...
        while aux!=None and not encontrado:
            if isinstance(aux.getVehiculo(),Usado):
                ...
                if unUsado.getPatente()==patente:
                    unUsado.setPrecio(float(input('Ingrese un nuevo precio: ')))
                    encontrado=True
